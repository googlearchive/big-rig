...
global.tr.exportTo('tr.e.rail', function() {
  ...
  RAILIRFinder.prototype = {
      ...
      openingEvents.forEach(function(openingEvent) {
        closingEvents.forEach(function(closingEvent) {
          ...
          if (isRenderFrameImplEvent(openingEvent)) {
            lir.renderProcessId = openingEvent.parentContainer.parent.pid;
    ...
    // parentRoutingId is the routingId of the createChildFrame event that
    // created the Load IR's RenderFrame.
    findLoadParentRoutingId_: function(lir) {
      function isCreateChildEvent(event) {
        return ((event.title === CREATE_CHILD_TITLE) &&
                (event.parentContainer.parent.pid === lir.renderProcessId) &&
                (event.args.child === lir.routingId));
      }

      var parentRoutingId = undefined;
      this.modelHelper.browserHelper.getAllAsyncSlicesMatching(
          isCreateChildEvent).forEach(function(event) {
        parentRoutingId = event.args.id;
      });

      return parentRoutingId;
    },
    ...
    findLoadInteractionRecords: function() {
      ...
      renderLIRs.push.apply(renderLIRs, failedLIRs);

      renderLIRs.forEach(function(lir) {
        // If a Load IR has a parentRoutingId, then it is in a subframe.
        // Drop Subframe Loads for now.
